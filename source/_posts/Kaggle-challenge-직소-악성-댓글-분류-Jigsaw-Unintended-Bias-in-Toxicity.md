...
<img src="/image/jigsaw_competition.JPG" width="650" height="400"> 
...
## Define Function
- 아래에서 사용될 함수를 미리 정의해 두었다. 텍스트 데이터의 punctuation과 stopwords를 제거해주는 함수와 embedding과 lstm을 활용한 모델을 정의하였다.
...
## 1. Load Data
- 데이터는 train 데이터가 180만건, test 데이터가 9만7천건 정도로 이루어져 있다.
...
- 가장 기본적인 텍스트 전처리를 위하여 간단히 텍스트 내의 punctuation과 stopwords를 처음 위에서 정의한 함수를 호출하여 처리하였다.
- 워낙 데이터가 커서 함수 호출 시 처리 속도가 오래 걸린다. 그래서 속도를 위해 list comprehension과 lambda로 처리하였는데 그래도 처리까지 시간이 꽤 걸렸다.
